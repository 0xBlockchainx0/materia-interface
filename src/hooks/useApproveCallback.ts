...
export function useTokenApproveCallback(
  ...
  const approvalState: ApprovalState = useMemo(() => {
    ...
    if ((ethItem && approveWUSD && !isWUSD) || (!approveWUSD && ethItem)) return ApprovalState.APPROVED
...
export function useMPApproveCallback(
  amountToApprove?: CurrencyAmount,
  spender?: string
): [ApprovalState, () => Promise<void>] {
  const { account, chainId } = useActiveWeb3React()
...
export function useApproveCallbackFromTrade(trade?: Trade, token?: Token, allowedSlippage = 0) {
