...
import { useSushiswapTradeExactIn, useSushiswapTradeExactOut } from '../../hooks/Trades'
...
import { sushiswapToMateriaTrade } from '../../utils/utilsSushiswap'

export function useBatchSwapState(): AppState['batchswapSushi'] {
  return useSelector<AppState, AppState['batchswapSushi']>(state => state.batchswapSushi)
}
...
export function useDerivedBatchSwapInfo(
  ...
  const inputParsedAmount = tryParseAmount(inputTypedValue, inputCurrency ?? undefined)
  const parsedAmount = tryParseAmount(typedValue, (isExactIn ? inputCurrency : outputCurrency) ?? undefined)

  const bestTradeExactIn = useSushiswapTradeExactIn(isExactIn ? parsedAmount : undefined, outputCurrency ?? undefined)
  const bestTradeExactOut = useSushiswapTradeExactOut(inputCurrency ?? undefined, !isExactIn ? parsedAmount : undefined)

  const sushiswapTrade = isExactIn ? bestTradeExactIn : bestTradeExactOut
  const v2Trade = sushiswapTrade ? sushiswapToMateriaTrade(sushiswapTrade, chainId ?? ChainId.MAINNET) : undefined
  ...
  const slippageAdjustedAmounts =
    ...
    computeBatchSwapSlippageAdjustedAmounts(v2Trade, allowedSlippage, outputField)
...
export function useValidateBatchSwapParameters(
  ...
  const {
    [Field.INPUT]: { typedValue: inputTypedValue, currency: inputCurrency }
  } = batchSwapState
  ...
  outputFields.map(outputField => {
    const {
      [outputField]: { typedValue: outputTypedValue, currency: outputCurrency, trade: outputHasTrade }
    } = batchSwapState
  ...
  const inputQuantityInvalid =
    inputTypedValue == undefined || inputTypedValue.trim().length == 0 || inputTypedValue == '0'
