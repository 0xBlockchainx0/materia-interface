...
import { useUniswapTradeExactIn, useUniswapTradeExactOut } from '../../hooks/Trades'
...
import { uniswapToMateriaTrade } from '../../utils/utilsUniswap'

export function useBatchSwapState(): AppState['batchswapUni'] {
  return useSelector<AppState, AppState['batchswapUni']>(state => state.batchswapUni)
}
...
export function useDerivedBatchSwapInfo(
  ...
  const uniswapTrade = isExactIn ? bestTradeExactIn : bestTradeExactOut
  const v2Trade = uniswapTrade ? uniswapToMateriaTrade(uniswapTrade, chainId ?? ChainId.MAINNET) : undefined
